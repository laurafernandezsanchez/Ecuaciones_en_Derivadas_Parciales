...
\]
de modo que la ecuación \eqref{eq:principal} se exprese de forma más sencilla. Usando la regla de la cadena para $u(x, y) = \mu(\xi, \eta)$:
...
Sustituyendo en \eqref{eq:principal}, obtenemos la ecuación transformada:
\begin{equation}
...
\begin{equation}
    a_{11}(z_x)^2 + 2a_{12}z_x z_y + a_{22}(z_y)^2 = 0 \label{eq:caracteristica_pde}
\end{equation}
...
\begin{proof}
    ...
    Multiplicando por $(\varphi_y)^2$ obtenemos \eqref{eq:caracteristica_pde}.
\end{proof}
...
\subsection{Ejemplos Preliminares}

\begin{ejemplo}{Ejemplo 1: Dependencia de los Datos (Mal Planteamiento)}
    ...
    \end{cases}
    \]
    Sabemos que la solución general de $u_{xt}=0$ es $u(x,t) = \varphi(x) + \psi(t)$.
    Imponemos las condiciones iniciales:
    \begin{enumerate}
        \item $u(x,0) = \varphi(x) + \psi(0) = \phi_0(x) \implies \varphi(x) = \phi_0(x) - C$.
        \item $u_t(x,t) = \psi'(t) \implies u_t(x,0) = \psi'(0)$.
    \end{enumerate}
    \textbf{Conclusión:} Para que exista solución, el dato $\phi_1(x)$ debe cumplir $\phi_1(x) = \psi'(0) = \text{cte}$.
    \begin{itemize}
        \item Si $\phi_1(x)$ no es constante: \textbf{No existe solución}.
        \item Si $\phi_1(x) = K$: Existen \textbf{infinitas soluciones} (indeterminación en $\psi(t)$ salvo en $t=0$).
    \end{itemize}
\end{ejemplo}

\begin{ejemplo}{Ejemplo 2: Compatibilidad y Analiticidad}
...
\section{Anexo: Funciones Analíticas y Multiíndices}

\begin{definicion}{Multiíndice}
    Un multiíndice $\alpha = (\alpha_1, \dots, \alpha_n) \in \N_0^n$ define:
    $|\alpha| = \sum \alpha_i$, $x^\alpha = x_1^{\alpha_1} \dots x_n^{\alpha_n}$ y $D^\alpha f = \frac{\partial^{|\alpha|} f}{\partial x_1^{\alpha_1} \dots \partial x_n^{\alpha_n}}$.
\end{definicion}

\begin{definicion}{Función Analítica}
    $f$ es analítica en $x_0$ si se puede expresar como una serie de potencias convergente:
    \[ f(x) = \sum_{\alpha \in \N_0^n} c_\alpha (x - x_0)^\alpha \]
\end{definicion}